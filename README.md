...
Live Notifications: Get a custom "Milk Medal Earned!" toast notification when you achieve the time. (I also want to add a small audio cue but I can't understand how it works. Help is always welcome!
...
Toggle the standalone HUD and the ability to Show/Hide deltas. 

📈Goals 

-Add custom medal PNG on campaign menu.

-Make it so this medal can get used by challenges plugins (Bingo, RACC, etc.). 

-Create my own "challenge" plugin that allows instant skip to the next map once the medal is achieved by one player in the server. (Feel free to make it yourself if you want; contact me so we can add this tier as well!) 

👥Credits 

This is my first project not only on OpenPlanet but on coding as a whole it was a blast! 
